...
namespace GSnapshot
{
    public class Disks
    {
        ...
        public async Task<ComputeData.Disk> GetRegionalDiskAsync(string projectId, string zone, string disk)
        {
            _logger.LogDebug($"Getting regional persistent disk information: {disk}");
            ComputeV1.RegionDisksResource.GetRequest request = computeService.RegionDisks.Get(projectId, zone, disk);
            ComputeData.Disk response = await request.ExecuteAsync();
            return response;
        }

        // Fetches the disk object based on an AttachedDisk object
        public async Task<ComputeData.Disk> GetDiskFromAttachedDiskAsync(string projectId, ComputeData.AttachedDisk disk)
        {
        ...
        public async Task<ComputeData.Disk?> _CreateDiskFromSnapshotAsync(string projectId, string diskName, ComputeData.Disk originalDisk, ComputeData.Snapshot snapshot, string snapshotDiskType, bool eraseVss)
        {
            ...
            if (!regional)
            {
                requestBody.Type = $"projects/{projectId}/zones/{_utils.GetLastPart(originalDisk.Zone)}/diskTypes/{snapshotDiskType}";
            }
            else
            {
                requestBody.Type = $"projects/{projectId}/regions/{_utils.GetLastPart(originalDisk.Region)}/diskTypes/{snapshotDiskType}";
            ...
            if (!regional)
            {
                ...
                _logger.LogInformation($"Creating new zonal disk {diskName} in {zone} from snapshot {snapshot.Name}...");
                ComputeBeta.DisksResource.InsertRequest request = betaComputeService.Disks.Insert(requestBody, projectId, zone);
                ...
                {
                    string done = await _operations.WaitOrFailForOperation(projectId, "zone", zone, _utils.GetLastPart(response.SelfLink));
                    if (done == "INTERNAL_ERROR") // Disk probably didn't have a VSS signature
                    {
                        _logger.LogDebug("Disk creation resulted in internal error, will retry with VSS signature erasing turned off...");
                        requestBody.EraseWindowsVssSignature = false;
                        request = betaComputeService.Disks.Insert(requestBody, projectId, zone);
                        response = await request.ExecuteAsync();
                        done = await _operations.WaitOrFailForOperation(projectId, "zone", zone, _utils.GetLastPart(response.SelfLink));
                    }
                    if (done == "")
                    {
                        operationCompleted = true;
                ...
                {
                    _logger.LogCritical($"Failed to create a zonal new disk, operation {response.SelfLink} failed to complete.");
                    return null;
                }
            }
            else
            {
                ...
                _logger.LogInformation($"Creating new regional disk {diskName} in {region} from snapshot {snapshot.Name}...");
                ComputeBeta.RegionDisksResource.InsertRequest request = betaComputeService.RegionDisks.Insert(requestBody, projectId, region);
                ComputeBetaData.Operation response = await request.ExecuteAsync();
                bool operationCompleted = false;
                ...
                {
                    string done = await _operations.WaitOrFailForOperation(projectId, "region", region, _utils.GetLastPart(response.SelfLink));
                    if (done == "INTERNAL_ERROR") // Disk probably didn't have a VSS signature
                    {
                        _logger.LogDebug("Disk creation resulted in internal error, will retry with VSS signature erasing turned off...");
                        requestBody.EraseWindowsVssSignature = false;
                        ...
                        response = await request.ExecuteAsync();
                        done = await _operations.WaitOrFailForOperation(projectId, "region", region, _utils.GetLastPart(response.SelfLink));
                    }
                    if (done == "")
                    {
                        operationCompleted = true;
                ...
                {
                    _logger.LogCritical($"Failed to create a new regional disk, operation {response.SelfLink} failed to complete.");
                    return null;
                }
            }
        }
        public async Task<ComputeData.Disk?> CreateDiskFromSnapshotAsync(string projectId, string diskName, ComputeData.Disk originalDisk, ComputeData.Snapshot snapshot, bool eraseVss)
        {
            var snapshotParts = snapshot.Name.Split('-');
            var snapshotDiskType = $"{snapshotParts[2]}-{snapshotParts[3]}";

            return await _CreateDiskFromSnapshotAsync(projectId, diskName, originalDisk, snapshot, snapshotDiskType, eraseVss);
        }

        // Creates all new disks from gsnapshot-style snapshots for an instance
        public async Task<Dictionary<string, ComputeData.Disk?>> CreateRollbackDisksAsync(string projectId, ComputeData.Instance instance, int id, bool forcePrevious, bool eraseVss)
        {
            ...
            Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>> disks = await _snapshots.GetSnapshotsForInstance(projectId, instance, id, forcePrevious);
            ...
            List<Task<Dictionary<string, ComputeData.Disk>>> tasks = new List<Task<Dictionary<string, ComputeData.Disk>>>();
            foreach (ComputeData.AttachedDisk attachedDisk in instance.Disks)
            {
                if (!disks.ContainsKey(attachedDisk.Source))
                {
                    _logger.LogWarning($"We don't have a snapshot for disk {_utils.GetLastPart(attachedDisk.Source)}, skipping...");
                    continue;
                }
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    if (snapshot != null)
                    {
                        var newDisk = await CreateDiskFromSnapshotAsync(projectId, newDiskName, disk, snapshot, eraseVss);
                        if (newDisk != null)
                        {
        ...
        public async Task<Dictionary<string, ComputeData.Disk?>> CreateScheduledRollbackDisksAsync(string projectId, ComputeData.Instance instance, Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot>> rollbackSnapshots, bool eraseVss)
        {
            Regex rx = new Regex(@"-[0-9]{8}-[0-9]{4}$", RegexOptions.Compiled);
            ...
            foreach (ComputeData.AttachedDisk attachedDisk in instance.Disks)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    if (snapshot != null)
                    {
                        ...
                        var newDisk = await _CreateDiskFromSnapshotAsync(projectId, newDiskName, disk, snapshot, _utils.GetLastPart(disk.Type), eraseVss);
                        if (newDisk != null)
                        {
                            result[disk.Name] = newDisk;
                        }
...
#pragma warning disable CS8602
        ...
        public async Task<bool> AttachNewDisksAsync(string projectId, string snapshotPrefix, ComputeData.Instance instance, Dictionary<string, ComputeData.Disk?> disks)
        {
            foreach (ComputeData.AttachedDisk disk in instance.Disks)
            {
                ...
                ComputeV1.InstancesResource.DetachDiskRequest request = computeService.Instances.DetachDisk(projectId, _utils.GetLastPart(instance.Zone), instance.Name, disk.DeviceName);
                ...
                if (diskType == "regions")
                {
                    ...
                    {
                        labelRequestBody.Labels["gsnapshot"] = labelRequestBody.Labels["gsnapshot"].Substring(0, 63);
                    }
                    labelRequestBody.LabelFingerprint = oldDisk.LabelFingerprint;
                    ComputeV1.RegionDisksResource.SetLabelsRequest labelRequest = computeService.RegionDisks.SetLabels(labelRequestBody, projectId, region, diskName);
                    ComputeData.Operation labelResponse = labelRequest.Execute();

                    done = await _operations.WaitForOperation(projectId, "region", region, _utils.GetLastPart(labelResponse.SelfLink));
                }
                else
                {
                    done = await _operations.WaitForOperation(projectId, "zone", zone, _utils.GetLastPart(response.SelfLink));
                    ...
                    ComputeV1.DisksResource.SetLabelsRequest labelRequest = computeService.Disks.SetLabels(labelRequestBody, projectId, zone, diskName);
                    ComputeData.Operation labelResponse = labelRequest.Execute();

                    done = await _operations.WaitForOperation(projectId, "zone", zone, _utils.GetLastPart(labelResponse.SelfLink));
                }
                ...
                ComputeV1.InstancesResource.AttachDiskRequest attachRequest = computeService.Instances.AttachDisk(attachRequestBody, projectId, _utils.GetLastPart(instance.Zone), instance.Name);
                ComputeData.Operation attachResponse = attachRequest.Execute();
                done = await _operations.WaitForOperation(projectId, "zone", _utils.GetLastPart(attachResponse.Zone), _utils.GetLastPart(attachResponse.SelfLink));
