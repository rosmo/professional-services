...
#pragma warning disable CS8604
        private void Snapshot(Options options)
        {
            ...
            _instances.CheckInstanceRunning(options.Project, _instances.AllInstances[options.Instance], options.Stop, "snapshot");
            Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot>> disks = _snapshots.SnapshotInstanceDisksAsync(options.Project, _instances.AllInstances[options.Instance], options.Location, options.ID, options.Force).GetAwaiter().GetResult();
            ...
            foreach (KeyValuePair<string, Tuple<ComputeData.Disk, ComputeData.Snapshot>> entry in disks)
            {
                if (entry.Value != null && entry.Value.Item1 != null && entry.Value.Item2 != null && entry.Value.Item2.StorageBytes != null)
                {
                    ...
                    string _snapshot = String.Format("{0,-" + snapshotNameLength + "}", entry.Value.Item2.Name);
                    _logger.LogInformation($"  Disk: {_disk}  Snapshot: {_snapshot}  Size: {size.ToString()}");
                }
            }
            var instance = _instances.GetInstance(options.Project, _instances.AllInstances[options.Instance]);
            if (instance.Status == "TERMINATED")
            {
                ...
                if (startServer)
                {
                    ...
                    bool instanceStarted = _instances.StartInstance(options.Project, instance).GetAwaiter().GetResult(); ;
                }
            }
        ...
        private void Rollback(Options options)
        {
            ...
            Dictionary<string, ComputeData.Disk?> newDisks = _disks.CreateRollbackDisksAsync(options.Project, _instances.AllInstances[options.Instance], options.ID, options.Force, options.EraseVSS).GetAwaiter().GetResult();
            _instances.CheckInstanceRunning(options.Project, _instances.AllInstances[options.Instance], options.Stop, "rollback");

            string snapshotName = $"gsnapshot-{options.ID}";
            bool done = _disks.AttachNewDisksAsync(options.Project, snapshotName, _instances.AllInstances[options.Instance], newDisks).GetAwaiter().GetResult();
            if (done)
            {
                _logger.LogInformation($"Rollback finished for snapshot #{options.ID}!");
                var rollbackInstance = _instances.GetInstance(options.Project, _instances.AllInstances[options.Instance]);
                if (rollbackInstance.Status == "TERMINATED")
                {
                    ...
                    if (startServer)
                    {
                        ...
                        bool instanceStarted = _instances.StartInstance(options.Project, _instances.AllInstances[options.Instance]).GetAwaiter().GetResult();
                    }
                }
            }
            else
        ...
        private void RollbackScheduled(Options options)
        {
            ...
            Dictionary<string, Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>>> snapshots = _snapshots.GetScheduledSnapshotsForInstance(options.Project, _instances.AllInstances[options.Instance]).GetAwaiter().GetResult();

            var rollbackInstance = _instances.GetInstance(options.Project, _instances.AllInstances[options.Instance]);

            Dictionary<string, string> allSnapshots = new Dictionary<string, string>();
            _logger.LogInformation($"Found the following scheduled snapshots for {rollbackInstance.Name}:");
            foreach (KeyValuePair<string, Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>>> entry in snapshots)
            {
                ...
                foreach (KeyValuePair<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>> disk in entry.Value)
                {
                    ...
                    if (disk.Value.Item2 != null)
                    {
                        ...
                        string timestamp = DateTimeOffset.Parse(disk.Value.Item2.CreationTimestamp).ToLocalTime().ToString();
                ...
                foreach (KeyValuePair<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>> disk in entry.Value)
                {
                    ...
                    if (disk.Value.Item2 != null && disk.Value.Item2.StorageBytes != null)
                    {
                        ...
                        string timestamp = String.Format("{0,-" + timestampLength + "}", DateTimeOffset.Parse(disk.Value.Item2.CreationTimestamp).ToLocalTime().ToString());

                        _logger.LogInformation($"    Disk: {_disk}  Snapshot: {_snapshot}  Timestamp: {timestamp}  Size: {size.ToString()}");
                    }
                    else
                    {
                        string _snapshot = String.Format("{0,-" + snapshotNameLength + "}", "N/A");
                        string timestamp = String.Format("{0,-" + timestampLength + "}", "N/A");
                        _logger.LogInformation($"    Disk: {_disk}  Snapshot: {_snapshot}  Timestamp: {timestamp}  Size: N/A");
                    }
                }
            }
            string snapshotId = "";
            ...
            if (String.IsNullOrEmpty(snapshotId))
            {
                ...
                var oldColor = Console.ForegroundColor;
                Console.ForegroundColor = ConsoleColor.White;
                snapshotId = ReadLine.Read("Snapshot ID for rollback (empty for all, tab to autocomplete, ^C to cancel)> ");
                Console.ForegroundColor = oldColor;
            }
            ...
            Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot>> rollbacks = new Dictionary<string, Tuple<ComputeData.Disk, ComputeData.Snapshot>>();
            foreach (KeyValuePair<string, Tuple<ComputeData.Disk, ComputeData.Snapshot?>> entry in snapshots[snapshotId])
            {
                rollbacks[entry.Key] = new Tuple<ComputeData.Disk, ComputeData.Snapshot>(entry.Value.Item1, entry.Value.Item2);
            }

            Dictionary<string, ComputeData.Disk?> newDisks = _disks.CreateScheduledRollbackDisksAsync(options.Project, _instances.AllInstances[options.Instance], rollbacks, options.EraseVSS).GetAwaiter().GetResult();

            _instances.CheckInstanceRunning(options.Project, _instances.AllInstances[options.Instance], options.Stop, "rollback");

            string snapshotPrefix = $"gsnapshot-scheduled-{snapshotId}";
            bool done = _disks.AttachNewDisksAsync(options.Project, snapshotPrefix, _instances.AllInstances[options.Instance], newDisks).GetAwaiter().GetResult();
            if (done)
            {
                _logger.LogInformation($"Rollback finished for snapshot ID {snapshotId}!");
                rollbackInstance = _instances.GetInstance(options.Project, _instances.AllInstances[options.Instance]);
                if (rollbackInstance.Status == "TERMINATED")
                {
                    bool startServer = options.Start;
                    if (!options.Start)
                    ...
                    {
                        _logger.LogInformation($"Starting instance {rollbackInstance.Name}...");
                        bool serverStarted = _instances.StartInstance(options.Project, _instances.AllInstances[options.Instance]).GetAwaiter().GetResult();
                    }
                }
            }
            else
        ...
        private void ScheduleSnapshot(Options options)
        {
            ...
            var instance = _instances.GetInstance(options.Project, _instances.AllInstances[options.Instance]);
            var schedule = _snapshots.GetSnapshotSchedule(options.Project, options.Schedule, options.Region).GetAwaiter().GetResult();
            if (schedule == null)
            {
            ...
            if (options.Force)
            {
                var clearDone = _snapshots.ClearInstanceSnapshotSchedule(options.Project, schedule.SelfLink, options.Region, _instances.AllInstances[options.Instance]).GetAwaiter().GetResult();
                if (clearDone)
                {
                ...
                {
                    _logger.LogWarning($"There were errors clearing snapshot schedules for instance {instance.Name}.");
                }
            }
            var done = _snapshots.AddInstanceSnapshotSchedule(options.Project, schedule.SelfLink, options.Region, _instances.AllInstances[options.Instance]).GetAwaiter().GetResult();
            if (done)
            {
            ...
            {
                _logger.LogWarning($"There were errors setting snapshot schedules for instance {instance.Name}.");
            }
        }
        public void Commit(Options options)
        {
            ...
            _snapshots.DeleteSnapshotsAsync(options.Project, _instances.AllInstances[options.Instance], options.ID, options.Delete).GetAwaiter().GetResult();
        ...
        public void DoAction(Options options)
        {
            ...
            switch (options.Command)
            {
                ...
                case "schedule-snapshot":
                    _logger.LogInformation("Operation specified: schedule snapshots (attach snapshot schedule to all of instance's disks)");
                    break;
                case "rollback":
                    _logger.LogInformation("Operation specified: rollback changes (create new disks from snapshots and attach)");
                    break;
                case "rollback-scheduled":
                    _logger.LogInformation("Operation specified: rollback to previous state (create new disks from scheduled snapshots and attach)");
                    break;
                case "commit":
                    _logger.LogInformation("Operation specified: commit changes (remove snapshots)");
                    break;
                default:
                    _logger.LogCritical("Command not selected, select one of: snapshot, schedule-snapshot, rollback, rollback-scheduled, commit");
                    ...
                    _logger.LogCritical("");
                    _logger.LogCritical("  snapshot             Takes a snapshot of a specified Compute Engine instance.");
                    _logger.LogCritical("  schedule-snapshot    Attaches a snapshot schedule to all disks of a Compute Engine instance.");
                    _logger.LogCritical("  rollback             Rolls back to a previously made snapshot.");
                    _logger.LogCritical("  rollback-scheduled   Rolls back to a previous snapshot from a snapshot schedule.");
            ...
            if (String.IsNullOrEmpty(options.Project))
            {
                ...
                if (String.IsNullOrEmpty(options.Project))
                {
                    _logger.LogCritical("Project ID was not set, either enter it manually or specify via command line.");
                    System.Environment.Exit(2);
                }
                else
                {
            ...
            if (String.IsNullOrEmpty(options.Region))
            {
                ...
                if (String.IsNullOrEmpty(options.Region))
                {
                    ...
                    _logger.LogWarning($"Region was not set, querying all regions ({RegionsToQuery.Count} regions).");
                }
                else
                {
            ...
            if (String.IsNullOrEmpty(options.Instance) || !_instances.AllInstances.ContainsKey(options.Instance))
            {
                _logger.LogCritical("Instance ID was not set, or set to non-existing instance.");
                System.Environment.Exit(4);
            }
            string description = _instances.GetMachineDescription(_instances.AllInstances[options.Instance]);
